...
exports.handler = async (event) => {
  ...
  try {
    ...
    const amount = Math.max(parseFloat(body.amount) || 0, 0);
    ...
    // Validation – minimum, aby to dávalo smysl pro potvrzení
    if (!email) return jsonError(400, "Chybí e-mail.");
    if (!street || !city || !zip) return jsonError(400, "Chybí adresa (ulice, město, PSČ).");
    if (amount <= 0) return jsonError(400, "Chybí nebo je neplatná částka daru.");
    ...
    if (!GSHEETS_SPREADSHEET_ID || !SERVICE_ACCOUNT_JSON_BASE64) {
      return jsonError(500, "Chybí nastavení pro ukládání do tabulky.");
    }
